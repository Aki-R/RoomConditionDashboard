...
with col1:
    st.metric(label="Temperature",
              value=f"{temperature_latest} °C",
              delta=f"{(temperature_latest - temperature_before):.1f} °C")

with col2:
    st.metric(label="Humidity",
              value=f"{humidity_latest} %",
              delta=f"{(humidity_latest - humidity_before):.1f} %")
...
if st.session_state.timescale == "1week":
    fig_temp.update_xaxes(range=[pd.Timestamp(datetime_before_week).to_pydatetime(),
                                 pd.Timestamp(datetime_latest).to_pydatetime()])
elif st.session_state.timescale == "1day":
    fig_temp.update_xaxes(range=[pd.Timestamp(datetime_before_day).to_pydatetime(),
                                 pd.Timestamp(datetime_latest).to_pydatetime()])

st.plotly_chart(fig_temp, theme="streamlit", use_container_width=True, config=config)
...
if st.session_state.timescale == "1week":
    fig_ilm.update_xaxes(range=[pd.Timestamp(datetime_before_week).to_pydatetime(),
                                pd.Timestamp(datetime_latest).to_pydatetime()])
elif st.session_state.timescale == "1day":
    fig_ilm.update_xaxes(range=[pd.Timestamp(datetime_before_day).to_pydatetime(),
                                pd.Timestamp(datetime_latest).to_pydatetime()])
